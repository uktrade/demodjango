...
@override_settings(BASIC_AUTH_USERNAME='valid_user', BASIC_AUTH_PASSWORD='valid_password')
def test_ipfilter_basic_auth_success(client):
    credentials = "valid_user:valid_password"
    encoded_credentials = base64.b64encode(credentials.encode('utf-8')).decode('utf-8')

    response = client.get('/ipfilter-basic-auth/', HTTP_AUTHORIZATION=f'Basic {encoded_credentials}')
...
def test_ipfilter_basic_auth_invalid_credentials(client):
    ...
    encoded_invalid_credentials = base64.b64encode(invalid_credentials.encode('utf-8')).decode('utf-8')

    response = client.get('/ipfilter-basic-auth/', HTTP_AUTHORIZATION=f'Basic {encoded_invalid_credentials}')
...
def test_ipfilter_basic_auth_invalid_auth_type(client):
    ...
    response = client.get('/ipfilter-basic-auth/', HTTP_AUTHORIZATION=f'Bearer {encoded_credentials}')
...
def test_ipfilter_basic_auth_malformed_auth_header(client):
    ...
    response = client.get('/ipfilter-basic-auth/', {"HTTP_AUTHORIZATION": malformed_header})
